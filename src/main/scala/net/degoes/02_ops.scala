...
object contact_processing {
  ...
  final case class SchemaMapping(map: ContactsCSV => MappingResult[ContactsCSV]) { self =>
    ...
    def +(that: SchemaMapping): SchemaMapping =
      SchemaMapping(contact => self.map(contact).flatMap(that.map))
    ...
    def orElse(that: SchemaMapping): SchemaMapping =
      SchemaMapping(contact => self.map(contact) orElse that.map(contact))
  ...
  object SchemaMapping {
     ...
     */
    def rename(oldName: String, newName: String): SchemaMapping = SchemaMapping { contacts =>
      val warnings = if (contacts.columnOf(oldName).isEmpty) List(s"Column '$oldName' doesn't exist") else List.empty
      MappingResult.Success(warnings, contacts.rename(oldName, newName))
    }
    ...
    ): SchemaMapping = SchemaMapping { contacts =>
      MappingResult.fromOption(
        contacts.combine(leftColumn, rightColumn)(newName)(f),
        s"Couldn't combine '$leftColumn' and '$rightColumn'"
      )
    }
     ...
     */
    def relocate(column: String, j: Int): SchemaMapping = SchemaMapping { contacts =>
      MappingResult.fromOption(contacts.relocate(column, j), s"Column '$column' doesn't exist")
    }
     ...
     * specified name.
     */
    def delete(name: String): SchemaMapping = SchemaMapping { contacts =>
      val warnings = if (contacts.columnOf(name).isEmpty) List(s"Column '$name' doesn't exist") else List.empty
      MappingResult.Success(warnings, contacts.delete(name))
    }
  }
...
object ui_events {
  ...
  final case class Listener(onEvent: GameEvent => Unit) { self =>
     ...
     */
    def debug: Listener = Listener.Debug + self
  }

  object Listener {
    val Debug: Listener = Listener(println)
  }
